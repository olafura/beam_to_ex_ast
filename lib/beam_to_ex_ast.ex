defmodule BeamToExAst do
        ...
        {mod_name, Enum.concat(Enum.map(body, fn
            {:clause, ln2, params, guard, body_def} ->
              case guard do
                ...
                [[g]] -> {:def, [line: ln2], [{:when, [line: ln2], [{name, [line: ln2], convert_params(params)}, convert_param(g)]}, def_body(body_def)]}
              end
            _ -> IO.inspect(body)
        end), rest)}
    end
    ...
    def def_caller({:remote, ln, {:atom, _ln, :erlang}, {:atom, ln2, :atom_to_binary}}, params) do
        {{:., [line: ln], [{:__aliases__, [counter: 0, line: ln2], [:Atom]}, :to_string]}, [line: ln2], List.delete_at(convert_params(params), -1)}
    end

    def def_caller({:remote, ln, {:atom, _ln, :erlang}, {:atom, ln2, :binary_to_atom}}, params) do
        {{:., [line: ln], [{:__aliases__, [counter: 0, line: ln2], [:String]}, :to_atom]}, [line: ln2], List.delete_at(convert_params(params), -1)}
    end

    def def_caller({:remote, ln, {:atom, _ln, :erlang}, {:atom, ln2, :binary_to_integer}}, params) do
        {{:., [line: ln], [{:__aliases__, [counter: 0, line: ln2], [:String]}, :to_integer]}, [line: ln2], convert_params(params)}
    end
    ...
    def def_caller({:var, ln, caller}, params) do
        {{:., [line: ln], [{clean_var(caller), [line: ln], nil}]}, [line: ln], convert_params(params)}
    end
    ...
    def remove_tuple({:{}, [line: ln], params}) do
        params
    end
    ...
    def convert_param({:var, ln, var}) do
        case Atom.to_string(var) do
            <<"_@", rest :: binary>> -> {:&, [line: ln], [String.to_integer(rest)]}
            _ -> {clean_var(var), [line: ln], nil}
        end
    end
    ...
    def convert_param({:tuple, ln, [{:atom, _ln, a1}, p2]}) do
        {a1, convert_param(p2)}
    end

    def convert_param({:tuple, ln, items}) when length(items) === 2 do
    ...
    def convert_param({:op, ln, op1, p1, p2}) do
        {clean_op(op1), [line: ln], [insert_line_number(convert_param(p1), ln), convert_param(p2)]}
    end

    def convert_param({:fun, ln, {:function, caller, number_of_params}}) do
        {:&, [line: ln], [{:/, [line: ln], [{caller, [line: ln], nil}, number_of_params]}]}
    end
    ...
    def convert_param({:case, ln, param, body}) do
        {:case, [line: ln], [convert_param(param), [do: Enum.map(body, &convert_param/1)]]}
    end
    ...
    def convert_param({:clause, ln, params, guard, body}) do
        case check_params(params) do
            false ->
                case guard do
                  [] -> {:->, [line: ln], [Enum.map(params, &convert_param_match/1), def_body_less_filter(body)]}
                  [[g]] -> {:->, [line: ln], [[{:when, [line: ln], [only_one(Enum.map(params, &convert_param_match/1)), convert_param(g)]}], def_body_less_filter(body)]}
                end
    ...
    def convert_param({:record, ln, name, params}) do
        {:record, [line: ln], [clean_atom(name), Enum.map(params, &convert_param/1)]}
    end

    def convert_param({:record_field, ln, {:atom, ln2, name}, p2}) do
    ...
    def convert_bin({:bin_element, ln, {:var, ln2, v1}, _, [_type]}) do
        convert_param({:var, ln2, v1})
    end

    def convert_bin({:bin_element, ln, {:var, ln2, v1}, _, :default}) do
        convert_param({:var, ln2, v1})
    end
