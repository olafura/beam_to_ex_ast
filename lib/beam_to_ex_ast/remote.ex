...
defimplEx BeamToExAst.Remote, {:remote, _ln, _param1, _param2}, for: Translate do
  ...
  def to_elixir({:remote, ln, {:atom, _ln, :erlang},
                  {:atom, ln2, :atom_to_binary}}, %{parents: [:call | _]} = opts) do
  ...
  def to_elixir({:remote, ln, {:atom, _ln, :erlang},
                  {:atom, ln2, :binary_to_atom}}, %{parents: [:call | _]} = opts) do
    opts = Map.update!(opts, :parents, &([:remote | &1]))
    {params, opts} = Map.pop(opts, :call_params)
    {{:., [line: ln],
  ...
  def to_elixir({:remote, ln, {:atom, _ln, :erlang},
                  {:atom, ln2, :binary_to_integer}}, %{parents: [:call | _]} = opts) do
    opts = Map.update!(opts, :parents, &([:remote | &1]))
    {params, opts} = Map.pop(opts, :call_params)
    {{:., [line: ln],
      [{:__aliases__, [line: ln2], [:String]}, :to_integer]},
     [line: ln2], Translate.to_elixir(params, opts)}
  end
  def to_elixir({:remote, ln,{:atom, _, mod_call},
